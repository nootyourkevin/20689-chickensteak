#!/usr/bin/env python3
"""端侧AI语言学习机 — Line C v2 入口（测试版）。

这是改进版 main.py，支持 SenseVoice ASR 测试版（立体声转单声道）。

启动方式：
    python src/main_test.py --voice --asr sensevoice_test --llm cloud
...
from line_c.asr.asrtest import SenseVoiceASRTest
...
def main():
    parser = argparse.ArgumentParser(description="端侧AI语言学习机 - Line C v2 (测试版)")
    parser.add_argument(
    ...
    parser.add_argument(
        "--asr", choices=["mock", "sensevoice", "sensevoice_test"], default="mock",
        help="ASR 后端选择 (默认: mock, sensevoice_test=改进版)"
    )
    parser.add_argument(
    ...
    if args.voice:
        ...
        elif args.asr == "sensevoice_test":
            asr = SenseVoiceASRTest(verbose=True)
            if not asr.is_available():
                print("警告：SenseVoice ASR Test 不可用，请检查模型路径")
        else:
